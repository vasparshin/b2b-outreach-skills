...
1. **Person** — LinkedIn `get_person_profile(slug)`: current role, tenure, headline, and **recent activity/posts** (the best personalization hook). If an Apollo contact is supplied, `apollo_people_match` for title/seniority. **If the supplied (Apollo) title disagrees with the LinkedIn headline, flag the drift and lower Quick-Facts confidence** — Apollo titles can be stale (e.g. post-acquisition the person still shows their old founder/co-founder headline).
2. **Company** — LinkedIn `get_company_profile` + `get_company_posts` (top ~3); Apollo `organizations_enrich` for size/funding/industry.
3. **Tech / AI-tooling signals** — the highest-value signals often live in **LinkedIn company posts** (`get_company_posts`): a hosted AI-coding event (a Claude Code / Cursor meetup), a first-party tool built *for* AI coding agents, an engineer demoing AI-coding workflows. **Check these first — they don't depend on web search** (which is frequently down). Then Apollo `organizations_job_postings` (hiring for AI/ML/platform/eng roles = adoption signal) + a Firecrawl/Exa search on the company site/careers/news for stack & AI initiatives.
...
6. **ICP fit verdict** — given the project's ICP (from config), is this person/company actually a *buyer*? **Strong / Moderate / Weak / Not-a-fit** + one-line rationale + confidence. **The Strong/Moderate divider:** Strong requires a *concrete public signal that their own engineers use the relevant tools* — and it does **NOT have to come from the prospect personally**; a clear company-level signal counts when this person owns that eng org. Qualifying signals: an engineering blog or a leader's post on AI-coding adoption; a stated adoption %/goal; job posts naming the tools; **a company-hosted AI-coding event (a Claude Code / Cursor meetup), a first-party product built for AI coding agents, or an engineer publicly demoing AI-coding workflows** — these are proof of real internal use, not just marketing. Right company-type + right title + right size but **no such signal at all → cap at Moderate**, however good it looks. (Don't over-discount a strong company signal just because the named person hasn't personally posted it — that mistake systematically under-rates good fits.) **Watch the trap:** a company that builds its own AI *product* is NOT automatically a fit — judge whether *their own engineers/users* would feel the pain the offer solves, at enough scale to matter, and whether THIS person owns/champions that area. Be willing to say "not a fit" — a clear disqualification is more valuable than a forced pitch.
